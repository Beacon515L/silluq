...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; i++){
        ...
        if(string(argv[i]).compare("-c")&&i<argc-1) {chars = string(argv[++i]).substr(1,string::npos);
    ...
    if(urtexts.count(name)!=0){
        ...
        for (int i; i < ranges.length(); i++){
            switch (ranges.at(i)){
                ...
                default: if(a!=0&&b!=0)
                  try{  if(range&&a<b) for(int j = a-1; j<=b; j++) urtexts[name]->at(j).uses++;
                            else urtexts[name]->at(a-1).uses++;}
